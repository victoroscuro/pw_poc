    ...
    if(await this.page.getByRole('link', { name: miniumName }).count() < 1) {
     {
        ...
        await expect(this.page.frameLocator('iframe[title="Add Site"]').getByText('The creation of the site may take some time. Closing the window will not cancel')).toBeHidden({ timeout: 20000 });
        await expect(this.page.getByRole('heading', { name: 'Site Settings' })).toBeVisible({ timeout: 10000 });
